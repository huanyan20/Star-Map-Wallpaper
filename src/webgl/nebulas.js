...
export function setupNebulas(scene) {
  ...
  fetch('assets/nebulas.json')
      ...
      for (const item of config) {
        ...
        const vertexShader = `
          ...
          void main() {
            ...
            // Use viewMatrix only (NOT modelViewMatrix) so the plane's own
            // modelMatrix doesn't re-transform the already-projected (px,py).
            // This mirrors how milkyway.js works (its Points mesh has identity
            // modelMatrix, so modelViewMatrix == viewMatrix there).
        ...
        const fragmentShader = `
          ...
          void main() {
            ...
            // Use luminance as alpha so black-background PNGs are fully transparent
            // in dark regions and glow brightly where the nebula is bright.
    ...
    .catch(err => {
      console.log('Nebula decal system inactive (no config found):', err.message);
    });
}
