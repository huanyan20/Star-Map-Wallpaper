...
const STAR_CHUNKS = [
    // 加入 loadFov 提早觸發網路下載，避免縮放過快時因非同步下載延遲造成的「區塊瞬間彈出 (Pop-in)」
    { url: 'assets/stars_chunk_1.bin', loadFov: 120 * Math.PI / 180, maxFov: 60 * Math.PI / 180, loaded: false, promise: null, pointsMesh: null },
    { url: 'assets/stars_chunk_2.bin', loadFov: 60 * Math.PI / 180, maxFov: 30 * Math.PI / 180, loaded: false, promise: null, pointsMesh: null }
];
...
window.updateStarLOD = function (hFOV) {
    ...
    for (const mat of mats) {
        if (mat.uniforms.currentFov) mat.uniforms.currentFov.value = hFOV;
    }

    for (const chunk of STAR_CHUNKS) {
        // 使用 loadFov 來進行提早背景載入，確保使用者縮放到 maxFov 時，Mesh 已經在場景中待命
        // Shader 會自動保證它在 maxFov 之前完全透明，因此提早加入 Scene 絕對安全
        const triggerFov = chunk.loadFov || chunk.maxFov;
        if (hFOV <= triggerFov) {
            if (!chunk.loaded && !chunk.promise) {
                // Not loaded, fetch it
                chunk.promise = loadStarChunk(chunk.url).then(geo => {
                    const mat = starsMaterial.clone();
                    mat.uniforms = THREE.UniformsUtils.clone(starsMaterial.uniforms);
