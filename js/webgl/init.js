  ...
  const skyFragmentShader = `
        ...
        void main() {
            ...
            if (sz >= -0.015) { // Add a tiny margin below horizon for blending
                ...
                // 判斷天空視線是否面向光源(月亮)，動態壓暗背光面的天際線
                // [同步優化] 響應「另以角度要漸弱但不能無光」：與海浪保持完美同步
                vec3 viewDir = normalize(vec3(vx, vy, sz));
                float glowFactor = dot(normalize(viewDir.xy + vec2(0.001)), normalize(lightDir.xy + vec2(0.001))) * 0.5 + 0.5;
                float skyDimming = mix(1.0, mix(0.45, 1.0, pow(glowFactor, 1.5)), min(lightIntensity, 1.0));
                vec3 finalHorRGB = horRGB * skyDimming;
                
                if (alt < 0.25) {
                    float t = alt / 0.25;
                    t = pow(t, 0.65); // Ease-out curve for wider horizon glow
                    color = mix(finalHorRGB, midRGB, t);
                } else {
                ...
                // 移除早期程式碼中各種強制的壓暗與漸層邏輯
                // [完全整合] 動態跟隨天空漸層 (horRGB, topRGB) 計算出和諧的海水底色，不再寫死任何顏色！
                vec3 baseWater = horRGB * 0.05 + topRGB * 0.1 + vec3(0.001, 0.002, 0.005);
                
                // 天空背景的 Alpha 永遠為 1.0，防止網頁底色外漏。星空會藉由 Additive Blending 疊加在此底色之上
                // 上層的 3D 透底海水 (ocean.js) 會與此底色完美融合
                gl_FragColor = vec4(baseWater, 1.0);
            }
        }
    `;
