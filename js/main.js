...
canvas.addEventListener(
  ...
  (e) => {
    if (e.touches.length === 2) {
      ...
      if (lastTD) {
        hFOV *= lastTD / d;
        hFOV = Math.max(toRad(5), Math.min(toRad(185), hFOV));
      }
      lastTD = d;
...
const btnMap = {
  ...
  constellations: 'btn-constellations',
  conNames: 'btn-con-names',
  starNames: 'btn-star-names',
  milkyway: 'btn-milkyway',
  grid: 'btn-grid',
  equatorial: 'btn-equatorial',
};
...
function doHover(mx, my) {
  ...
  if (nearest) {
    ...
    document.getElementById('tt-meta').innerHTML =
      '星座: ' +
      (CON_NAMES[nearest.con] || nearest.con) +
      '<br>' +
      '<span class="star-mag">視星等: ' +
      nearest.mag.toFixed(2) +
      '</span><br>' +
      '光譜型: ' +
      (nearest.sp || '?') +
      '<br>' +
      'RA: ' +
      nearest.ra.toFixed(3) +
      'h &nbsp;Dec: ' +
      nearest.dec.toFixed(2) +
      '°';
...
function focalLen() {
  return W / 4 / Math.tan(hFOV / 4);
}
/* horizon Y in screen coords (Approximate stereographic horizon) */
function horizonY() {
  return CY + 2 * focalLen() * Math.tan(lookEl / 2);
}
