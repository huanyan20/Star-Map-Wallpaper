...
- **Atmospheric Scattering Performance**: Do NOT use brute-force raymarching (`getAtmosphere()` loops) per-pixel at full resolution every frame (e.g., 96+ steps for Rayleigh/Mie integration). In real-time engineering, always decouple the "expensive integration" from the "cheap sampling" by using Precomputed Look-Up Tables (LUTs) or lower-resolution off-screen passes.
