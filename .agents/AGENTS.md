...
## Context Initialization & Codebase Verification
...
- **CRITICAL**: After reviewing the architecture memory map, review any other relevant markdown (`.md`) files in the project (e.g., `README.md`, `docs/performance-plan.md`, `artandalo.md`, etc.) using the `view_file` tool to establish a complete understanding before performing edits.
- **CRITICAL**: You MUST actively verify that the current codebase (JavaScript/WebGL code) actually adheres to the architectural design, performance guidelines, and constraints defined in these `.md` documents. Do not assume compliance without checking key files (e.g., shaders, render loops).
...
## Codebase Analysis & Proactive Suggestions
- **Active MCP Reading**: When asked to read, analyze, or provide suggestions for the STAR project, you MUST actively query the `codebase-memory-mcp` tools (e.g., `get_architecture`, `search_graph`, `trace_path`, `query_graph`) to understand the current graph state of the codebase.
- **Suggestion Generation**: After reading the architecture via MCP, automatically formulate and propose actionable architectural or performance suggestions.
- **Focus Areas for Suggestions**:
  - Identify complex hotspots (e.g., `sky.js`, `render.js`, `stars.js`).
  - Propose strategies for GPU offloading, draw call minimization, or reducing fragment shader complexity.
  - Suggest refactoring opportunities based on the module dependencies retrieved from the knowledge graph.
